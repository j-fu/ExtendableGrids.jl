...
b) "grid_to_file": takes an ExtendableGrid and writes the grid into a msh file
...
function faces_of_ndim_simplex(x, dim, nn)
	...
	elseif dim == 2
		y = [encode(sort([x[1],x[2]]),nn), encode(sort([x[1],x[3]]),nn), encode(sort([x[2],x[3]]),nn)]
	end
	return y
...
function assemble_bfaces(simplices,dim,nn)
	...
	for i = 1:m
		poss_faces[(dim+1)*i-dim:(dim+1)*i] = faces_of_ndim_simplex(simplices[:,i], dim,nn)
	end
...
function mod_to_grid(model::Module, manualset::Int64)
	...
	return simplexgrid(
		coords_new, simplices, cellregions, bfaces, bfaceregions
		)
end
...
### this function writes an ExtendableGrid into a gmsh file, called "testwrite.msh"

function grid_to_file(grid::ExtendableGrid)
	gmsh.initialize()
